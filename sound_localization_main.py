...
def evaluate(model, writer, index, dataset_val, opt):
    ...
    with torch.no_grad():
        for i, (
            ...
            (
                z_val,
                pos_audio_embedding_val,
                neg_audio_embedding_val,
                att_map_val,
            ) = model.forward(frame_t_val, pos_audio_val, neg_audio_val)
            # Calculate the loss
            val_unsup_loss = unsupervised_loss_criteria(
                z_val, pos_audio_val, neg_audio_val, weights_t_val, opt
            )
...
dataloader_train = DataLoader(
    dataset_train, batch_size=opt.batchSize, num_workers=opt.nThreads, shuffle=True
)

# create validation set dataloader
if opt.validation_on:
    opt.mode = "val"
    dataset_test = Sound_Localization_Dataset(
        opt.val_dataset_file, "test", opt.annotation_path
    )
    dataloader_test = DataLoader(
        dataset_test, batch_size=32, shuffle=False, num_workers=opt.nThreads
    )  # , sampler = RandomSampler(dataset_test,replacement=True,num_samples=100))
...
for epoch in range(1 + opt.epoch_count, opt.niter + 1):
    ...
    for i, (
        ...
        z, pos_audio_embedding, neg_audio_embedding, att_map = model.forward(
            frame_t, pos_audio, neg_audio
        )
        # Calculate the loss
        ...
        print(
            ...
            "Loss {total_err.val:.7f} ({total_err.avg:.7f})\t".format(
                epoch,
                i + 1,
                len(dataloader_train),
                batch_time=batch_time,
                data_time=data_time,
                unsup_err=unsupervised_loss,
                bot_loss=bottomup_loss,
                total_err=batch_loss,
            )
        )

    if epoch % opt.display_freq == 0:
        print(
            "Display training progress at (epoch %d, total_epoch %d)"
            % (epoch, opt.niter)
        )
